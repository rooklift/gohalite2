...
func (self *Overmind) PlanetProblems(planet *hal.Planet) []*Problem {
	...
	default:
		...
		if planet.Owner == self.Game.Pid() || planet.Owned == false {

			// Defense / battle for neutral.
			...
			total_need := len(enemies) * 2

			docked_enemies := self.Game.ShipsDockedAt(planet)

			for _, enemy := range docked_enemies {

				if enemy.Doomed == false {
					ret = append(ret, &Problem{
						Entity: enemy,
						Value: 1.0,
						Need: total_need / len(docked_enemies),
						Message: planet.Id,
					})
				}
			}
...
func (self *Overmind) PlanetProblems_pre_v84(planet *hal.Planet) []*Problem {
